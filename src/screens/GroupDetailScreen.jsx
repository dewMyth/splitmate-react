...
export default function GroupDetailScreen({ navigate, params }) {
  const { deleteGroup, deleteExpense, showSnack } = useApp();
    ...
    <div className="screen">
      ...
      <div className="group-header">
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
          <button className="app-bar-back" onClick={() => navigate("home")}>
            ‹
          </button>
          <div style={{ flex: 1 }} />
          <button
      ...
      <div className="screen-body">
        {tab === "expenses" && (
          <ExpensesTab group={group} onDelete={(id) => setConfirmDelete(id)} />
        )}
  ...
  if (group?.expenses?.length === 0) {
    return (
      <EmptyState
        ...
        subtitle='Tap "Add Expense" to log your first shared expense'
      />
    );
  }
  return (
    <div style={{ padding: "16px 20px 120px" }}>
      {group?.expenses?.map((expense) => {
        const payer =
          group?.participants.find((p) => p.id === expense.payerId) ||
  ...
  if (group?.participants.length === 0) {
    return (
      <EmptyState
        ...
        subtitle="Add participants to see balances"
      />
    );
  }
  ...
  if (settlements.length === 0) {
    return (
      <EmptyState
        ...
        title={
          group?.expenses?.length === 0 ? "No expenses yet" : "All settled up!"
        }
        subtitle={
            ...
            >
              ...
              <div
                style={{
                  fontSize: 14,
                  fontWeight: 600,
                  color: bal >= 0 ? "var(--positive)" : "var(--negative)",
                }}
              >
                {bal >= 0 ? "+" : "-"}
